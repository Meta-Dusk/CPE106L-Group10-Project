...
async def animated_slide_out(control: ft.Control, duration_in_milliseconds: int = 500):
    control.animate_offset = ft.Animation(duration=duration_in_milliseconds, curve=ft.AnimationCurve.EASE_IN_OUT)
    control.animate_opacity = ft.Animation(duration=duration_in_milliseconds)
    control.animate_rotation = ft.Animation(duration=duration_in_milliseconds, curve=ft.AnimationCurve.EASE_IN_OUT)
    control.offset = ft.Offset(1.5, 0.2)                       # Slide right and go up slightly
    control.opacity = 0.0
    control.rotate = ft.Rotate(-0.2, ft.alignment.bottom_left) # Rotate a bit on the bottom left anchor
    control.update()
...
async def prepare_for_slide_in(control: ft.Control):
    # Disable animation
    control.animate_offset = 0
    control.animate_rotation = 0
    
    # Set origin points
    control.offset = ft.Offset(-1.5, 0)
    control.update()
...
async def animate_slide_in(control: ft.Control, duration_in_milliseconds: int = 500):
    control.animate_offset = ft.Animation(duration=duration_in_milliseconds, curve=ft.AnimationCurve.EASE_IN_OUT)
    control.animate_opacity = ft.Animation(duration=duration_in_milliseconds)
    control.animate_rotation = ft.Animation(duration=duration_in_milliseconds)
    control.opacity = 1.0
...
async def teeter_right(control: ft.Control, duration_in_milliseconds: int = 500):
    control.animate_offset = ft.Animation(duration=duration_in_milliseconds, curve=ft.AnimationCurve.EASE_IN_OUT)
    control.animate_rotation = ft.Animation(duration=duration_in_milliseconds)
    control.offset = ft.Offset(0.0, 0.2)
    control.rotate = ft.Rotate(0.2, ft.alignment.bottom_right)
    control.update()
